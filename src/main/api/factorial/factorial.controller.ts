...
export async function getFactorial(
    factorialNumberToCalculate: number
): Promise<string | null> {
    try {
        const response = await FactorialService.getFactorial(
            factorialNumberToCalculate
        );
        if (response) {
            return response;
        }
        logger.error(
            `Unable to get factorial for the number: ${factorialNumberToCalculate}`
        );
        return null;
    } catch (err) {
